...
static int g_GBA_Density = 0;
static char *g_GBA_RomFile = "";

float SDLCALL SDL_GetDensity()
{
	return (float)g_GBA_Density;
}

char * SDLCALL SDL_GetRomFile()
{
	return g_GBA_RomFile;
}

int SDLCALL SDL_FinishActivity()
{
	JNIEnv* mEnv = Android_JNI_GetEnv();
	(*mEnv)->CallStaticVoidMethod(mEnv, mActivityClass, (*mEnv)->GetStaticMethodID(mEnv, mActivityClass, "FinishActivity", "()V"));
}
...
void SDL_Android_Init(JNIEnv* mEnv, jclass cls)
{
	...
	g_GBA_Density = (*mEnv)->CallStaticIntMethod(mEnv, mActivityClass, (*mEnv)->GetStaticMethodID(mEnv, mActivityClass, "GetDensity", "()I"));
	jstring romFile = (*mEnv)->CallStaticObjectMethod(mEnv, mActivityClass, (*mEnv)->GetStaticMethodID(mEnv, mActivityClass, "GetRomFile", "()Ljava/lang/String;"));
	const char * strRomFile = (*mEnv)->GetStringUTFChars(mEnv, romFile, 0);
	g_GBA_RomFile = strdup(strRomFile);
	(*mEnv)->ReleaseStringUTFChars(mEnv, romFile, strRomFile);
